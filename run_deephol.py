...
def convert_single_example(ex_index, example, tac_label_list, is_negative_label_list, max_seq_length,
                           tokenizer):
  ...
  if ex_index < 5:
    ...
    tf.logging.info("goal_tokens: %s" % " ".join(
        [tokenization.printable_text(x) for x in goal_tokens]))
    tf.logging.info("thm_tokens: %s" % " ".join(
        [tokenization.printable_text(x) for x in thm_tokens]))
    tf.logging.info("goal_input_ids: %s" % " ".join([str(x) for x in goal_input_ids]))
    ...
    tf.logging.info("goal_segment_ids: %s" % " ".join([str(x) for x in goal_segment_ids]))
    tf.logging.info("thm_segment_ids: %s" % " ".join([str(x) for x in thm_segment_ids]))
...
def tactic_classifier(goal_net, is_training, tac_labels, num_tac_labels):
  ...
  with tf.variable_scope("loss"):
    ...
    goal_net = tf.layers.dense(goal_net, hidden_size, activation=tf.nn.relu, name='tac_dense1')

    if is_training:
      goal_net = tf.nn.dropout(goal_net, keep_prob=0.7)
    goal_net = tf.layers.dense(goal_net, hidden_size, activation=tf.nn.relu, name='tac_dense2')

    if is_training:
      goal_net = tf.nn.dropout(goal_net, keep_prob=0.7)
    tac_logits = tf.layers.dense(goal_net, num_tac_labels, activation=tf.nn.relu, name='tac_dense3')
...
def pairwise_classifier(goal_net, thm_net, is_training, is_negative_labels, tac_labels):
  ...
  with tf.variable_scope("loss"):
    ...
    net = tf.layers.dense(net, hidden_size, activation=tf.nn.relu, name='par_dense1')

    if is_training:
      net = tf.nn.dropout(net, keep_prob=0.7)
    net = tf.layers.dense(net, hidden_size, activation=tf.nn.relu, name='par_dense2')
