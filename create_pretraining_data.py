...
def create_masked_lm_predictions(tokens, masked_lm_prob,
  ...
  back_to_cover = round(len(tokens) - 2) * bask_mask_prob

  idx = len(tokens) - 2

  while (idx >= len(tokens) - int (back_to_cover) - 2):
  ...
  bck_masked = len(masked_lms)

  num_to_predict = min(max_predictions_per_seq - bck_masked,
                       max(1, int(round(len(tokens) * masked_lm_prob))))

  covered_indexes = set()
  for index_set in cand_indexes:
    if len(masked_lms) >= num_to_predict + bck_masked:
      break
    # If adding a whole-word mask would exceed the maximum number of
    # predictions, then just skip this candidate.
    if len(masked_lms) + len(index_set) > num_to_predict + bck_masked:
      continue
  ...
  assert len(masked_lms) <= num_to_predict + bck_masked
