...
def create_masked_lm_predictions(tokens, masked_lm_prob,
  ...
  front_mask_prob = 0.05
  masked_lms = []
  output_tokens = list(tokens)
  front_to_cover = round(len(tokens) - 2) * front_mask_prob

  idx = 1

  while (idx < int (front_to_cover)):
  ...
  fr_masked = len(masked_lms)

  num_to_predict = min(max_predictions_per_seq - fr_masked,
                       max(1, int(round(len(tokens) * masked_lm_prob))))

  covered_indexes = set()
  for index_set in cand_indexes:
    if len(masked_lms) >= num_to_predict + fr_masked:
      break
    # If adding a whole-word mask would exceed the maximum number of
    # predictions, then just skip this candidate.
    if len(masked_lms) + len(index_set) > num_to_predict + fr_masked:
      continue
  ...
  assert len(masked_lms) <= num_to_predict + fr_masked
